...
interface UseProductFrameSequenceOptions {
  totalFrames: number;
  framePath: string;
  frameExt: string;
  bgColor: string;
...
interface UseProductFrameSequenceReturn {
...
export function useProductFrameSequence({
  totalFrames,
  framePath,
  frameExt,
  bgColor,
}: UseProductFrameSequenceOptions): UseProductFrameSequenceReturn {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < totalFrames; i++) {
      ...
      img.onerror = () => {
        console.warn(`Product Frame ${i + 1} failed to load`);
        onDone();
      };
    }
